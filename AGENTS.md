...
- **Every issue that asserts behavior carries a `## Reproduction` section holding one complete, minimal, runnable example.** A snippet naming models the reader has to invent is not a reproduction — it demonstrates that the author understood the problem, not that anyone else can see it. The section carries a single self-contained script (imports, model definitions, database setup, and the calls that show the behavior), the exact command that runs it, the output it actually printed, the output expected instead, and the environment it ran in: commit, date, Python version, and the version of each dependency the behavior depends on.
  - **Minimal means minimal.** Cut every model, field, call, and line of output not required to show the behavior — anything left over competes with the finding for the reader's attention. Prefer `MemoryStorage` unless the point is on-disk state, one model over two, one field over three, and literal values over anything computed. If a line can be deleted and the output still makes the point, delete it.
  - **Run the script, then paste what it printed.** A reproduction written from memory is exactly what this rule exists to prevent. If it could not be run, the issue says so plainly — see the "say what you could _not_ do" rule below.
  - **A performance finding's reproduction is its measurement harness** — the storage, the table size, the number of runs, and the numbers it produced — on the same terms: runnable, minimal, and actually run.
  - **When there is genuinely nothing to run** — a docs page that does not exist, a decision to record — keep the heading and put the evidence under it instead: the command searched, the paths it covered, and its output.
  - The [issue forms](https://github.com/tinydantic/tinydantic/tree/main/.github/ISSUE_TEMPLATE) in `.github/ISSUE_TEMPLATE/` carry these fields for issues filed through the web UI. An issue filed with `gh` supplies the same sections by hand.
...
- A PR that addresses an issue must link it with a closing keyword (`Closes #123`, `Fixes #123`) in the PR body, so merging the PR closes the issue. Only use a closing keyword when merging genuinely resolves that issue — it fires on merge and will close an issue someone has deliberately reopened.
- **Never write an `@` mention anywhere in this repo** — issue bodies, comments, PR bodies, commit messages. GitHub turns it into a real notification to that person, and **quoting does not suppress it**: a draft inside a `>` quote still pulls them in, and so does one under a heading that says "do not post". This is not hypothetical; it has happened here, when draft collateral moved out of a git-ignored file into an issue and notified an upstream maintainer about a reply that was never sent. Text that is inert in a scratch file goes live the moment it is pasted into GitHub. When draft material has to quote a conversation, write the name without the `@`, or wrap the whole token in a code span — GitHub does not create mentions inside code.
...
- Referencing an upstream issue (`owner/repo#123`, or its URL) posts a visible cross-reference on **their** tracker. A genuine tracking link is worth that and this repo has always done it — but it is not invisible, so keep upstream references to the places that need them and use a code span in quoted draft text. Combined with the `@handle` rule above: assume anything you paste into a public issue here can reach the people it names.
