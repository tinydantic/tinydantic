...
- Computed fields are now queryable through the `Model.field` shorthand: `Product.price_band == "budget"` builds a real TinyDB query instead of silently evaluating to `False`. `field(Model, name)` already accepted computed fields (they are serialized, so they reach storage as ordinary document keys), but attribute access could not reach them — pydantic leaves a computed field as a real class attribute, so ordinary lookup succeeds and the metaclass `__getattr__` that powers `Model.field` never fires. Class-level access now resolves through a descriptor, which sees the distinction `__getattr__` cannot. Instance access (`user.price_band`), serialization, JSON schema, and inheritance are unchanged, and computed fields stay read-only: assignment is routed to the wrapped property, so a computed field with a setter still runs it and one without still raises `AttributeError: can't set attribute`. Frozen models are left to pydantic's own frozen-instance error. Ordinary (non-`computed_field`) properties are deliberately untouched — they are not serialized, so a query on one could never match.
