...
**Why it matters to an ODM.** `update_multiple()` is TinyDB's only batched write — the whole batch runs in one atomic read-modify-write cycle (`Table._update_table()`), which is the reason to use it over looping `update()`. `tinydantic` translates id conditions to `doc_ids=` operations everywhere else, but `update_multiple()` offers no landing spot for that translation, so `TinydanticModel.update_multiple()` currently raises `DocumentIDConditionError` for id conditions and directs users to per-condition `update()` calls — which sacrifices the batch's single-write atomicity.
...
| --- | --- | --- | --- | --- |
| `Table._update_table(updater)` | `TinydanticModel.update_multiple()` (id-condition batches) | **Proposed — pending review, not yet implemented** | The only way to run a batched update selecting documents by id in one atomic read-modify-write cycle: `update_multiple()` has no `doc_ids` parameter, and conditions cannot see ids (see the two limitations above). A custom updater evaluates conditions against `Document(body, doc_id)` wrappers and applies the same mutation logic as upstream's updater, inside the same read → mutate → write → cache-clear lifecycle. | Either `update_multiple()` improvement above. |
