...
- Proposals to TinyDB are drafted in `untracked/upstream/` (git-ignored scratch space at the repository root) and are never filed on the TinyDB issue tracker without explicit approval.
