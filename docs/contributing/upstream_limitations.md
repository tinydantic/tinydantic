...
Existing queries and third-party `QueryLike` objects pay nothing (the `getattr` default preserves current behavior); id queries work in every API including `update_many()`; the storage format is untouched. This would let `tinydantic` delete its entire translation layer.
...
- Accept an optional per-pair id selector, e.g. `update_many(updates: Iterable[tuple[fields, cond | None, doc_ids | None]])` (or a parallel `update_multiple_by_ids()`), mirroring the `update()`/`remove()` signatures.
